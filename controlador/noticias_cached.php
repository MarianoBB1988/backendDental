<?php
// Cache simple para noticias
// Cachea las noticias por 30 minutos

$cacheFile = __DIR__ . '/../log/noticias_cache.json';
$cacheTime = 30 * 60; // 30 minutos

// Verificar si existe cache válido
if (file_exists($cacheFile) && (time() - filemtime($cacheFile) < $cacheTime)) {
    error_log("[NOTICIAS] Usando cache existente");
    $cachedData = file_get_contents($cacheFile);
    echo $cachedData;
    exit;
}

// Si no hay cache válido, hacer scraping y guardar en cache
error_log("[NOTICIAS] Cache expirado o no existe, haciendo scraping");
...
$result = json_encode(['noticias' => $noticias], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);

// Guardar en cache
file_put_contents($cacheFile, $result);
error_log("[NOTICIAS] Cache guardado en: " . $cacheFile);
